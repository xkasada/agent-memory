...
def raw_project_kind(rel_path: str) -> str:
    ...
    `Raw/Projects/<name>/<sub>/*.md`      -> PROJECT_SUB_KIND[<sub>] or <sub> (generic:
                                             any project subfolder is a kind)
    `Raw/Projects/<name>/packs/*.md`      -> packs (skipped)
    """
    parts = str(rel_path).replace("\\", "/").split("/")
    if len(parts) == 4:
        return "index" if Path(parts[3]).stem.lower() == "index" else "context"
    if len(parts) >= 5:
        sub = parts[3]
        if sub == "packs":
            return "packs"
        return PROJECT_SUB_KIND.get(sub, re.sub(r"[^a-z0-9_-]", "", sub.lower()))
    return ""
...
def cmd_build(args):
    ...
    if projects:
        print(f"index  : wrote {len(projects)} per-project index file(s) under {rel(WIKI_PROJECTS)}/")
    return 0
...
def raw_project_sub(rel_path: str) -> str:
    """Sub-folder under the project that the compiled note mirrors ('' for top level)."""
    parts = str(rel_path).replace("\\", "/").split("/")
    if len(parts) >= 5 and parts[3] != "packs":
        return parts[3]
    return ""
...
def render_project_note(title: str, kind: str, tag: str, name: str, src, anchor_title=None) -> str:
    anchor_title = anchor_title or name
    ...
    lines = [
        ...
        "## Connections",
        "",
        f"- [[{anchor_title}]] — component of project {name}",
        "",
        "## Sources",
        "",
...
def compile_project(name: str, dry_run: bool):
    ...
    for s in raws:
        ...
        title = Path(s["rel"]).stem if kind in ("context", "module", "artifact") else _compile_title(s)
        if title.lower() in seen:
            title = f"{title} ({Path(s['rel']).parent.name})"
        seen.add(title.lower())
        title = _unique_project_title(title, name)
        seen.add(title.lower())
        out_path = WIKI_PROJECTS / name / sub / f"{title}.md"
        expected.add(out_path)
        if not dry_run:
            write_text(out_path, render_project_note(title, kind, tag, name, s))
        children.append({"title": title, "kind": kind, "summary": summary_of(s["body"])})
        written += 1
    anchor_path = WIKI_PROJECTS / name / f"{name}.md"
    expected.add(anchor_path)
    if not dry_run:
        shared = _anchor_shared_links(name, raws)
        write_text(anchor_path, render_project_anchor(name, raws, anchor_raw, children, shared))
        # Reconcile: drop compiled notes no longer produced (keep generated index.md).
